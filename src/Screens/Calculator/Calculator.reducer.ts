...
// @ts-ignore
export const calculatorReducer: Reducer<State, Action> = (state = initialState, {type, payload= {}}) => {
  ...
  switch (type) {
    case ACTIONS.ADD_VALUE:
      ...
      values[nextIndex] = 0;
      return {...state, values, total: calculateTotal(state.operator, values)};
    ...
    case ACTIONS.SET_OPERATOR:
      return {...state, values, operator, total: calculateTotal(operator, values)};
